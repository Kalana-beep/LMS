...
exports.login = async (req, res) => {
  try {
    ...
    res.json({ token, user: { id: user._id, name: user.name, email, role: user.role, subscriptionStatus: user.subscriptionStatus, labnetId: user.labnetId, blocked: user.blocked, blockReason: user.blockReason } });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
